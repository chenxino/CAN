...
def get_data_dim(dataset):
    ...
    if dataset == "SMAP":
        ...
        return 118
    elif dataset == 'SWaT_10':
        return 51
    elif dataset == 'WADI_10':
        return 118
    else:
...
def get_data(dataset, max_train_size=None, max_test_size=None,
    ...
    if str(dataset).startswith("machine"):
        ...
        prefix += "/data/processed"
        train_start=259200 if dataset == "WADI" else 21600
        if dataset == "WADI":
            max_train_size=789371
    elif dataset in ['SWaT_10', 'WADI_10']:
        prefix += "/data/processed"
        train_start=25920 if dataset == "WADI_10" else 2160
        if dataset == "WADI_10":
            max_train_size=78937
...
def get_value_from_train(train_dataset, val_split=0.1, shuffle=True, test_dataset=None):
    if val_split == 0.0:
        ...
        if shuffle:
            np.random.shuffle(indices)
        train_indices, val_indices = indices[split:], indices[:split]
        x_value = train_dataset[val_indices]
        return x_value